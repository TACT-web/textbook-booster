...
with tab1:
    ...
    with t_col1:
        st.title("🚀 教科書ブースター")
    with t_col2:
        subject_choice = st.selectbox("🎯 ターゲット教科", list(SUBJECT_PROMPTS.keys()), label_visibility="collapsed")
    ...
    # 全画面カメラ対応
    cam_file = st.file_uploader("📸 教科書をスキャン", type=['png', 'jpg', 'jpeg'], capture="camera")

    if cam_file and st.button("✨ ブースト開始", use_container_width=True):
        genai.configure(api_key=st.session_state.user_api_key)
        # 指定モデル名 gemini-3-flash-preview を維持
