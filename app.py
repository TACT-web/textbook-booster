...
with tab1:
    # タイトルの1行化（カラム利用）
    title_c1, title_c2 = st.columns([3, 1])
    with title_c1:
        st.title("🚀 教科書ブースター")
    with title_c2:
    ...
    # 標準カメラ呼び出し（ capture="camera" ）
    img_file = st.file_uploader("📸 教科書を撮影してください（全画面カメラ起動）", type=['png', 'jpg', 'jpeg'], capture="camera")
        ...
        if st.button("✨ ブースト開始", use_container_width=True):
            ...
            with st.status("解析中...🚀"):
                ...
                if not res_json.get("is_match"):
                    st.error(f"🚫 教科不一致または判別不能: {res_json.get('detected_subject', '不明')}")
                    st.stop()
    ...
    if st.session_state.final_json:
        ...
        with st.container(border=True):
            ...
            with col3:
                v_btn_txt = "🎙️ 個別音声:ON" if st.session_state.show_voice_btns else "🎙️ 個別音声:OFF"
                if st.button(v_btn_txt, use_container_width=True):
