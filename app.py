...
# [cite_start]--- 添付ファイル仕様継承：教科別個別プロンプト（完全復元 [cite: 1]） ---
SUBJECT_PROMPTS = {
...
# [cite_start]--- 音声合成エンジン（完全復元 [cite: 1, 2, 3, 4, 5, 6, 7]） ---
...
# [cite_start]--- 1. 冒頭：厳格な免責事項 ＆ 同意（完全復元 [cite: 8]） ---
...
with tab1:
    ...
    with t_col1:
        st.title("🚀 教科書ブースター")
    with t_col2:
        [cite_start]subject_choice = st.selectbox("🎯 教科", list(SUBJECT_PROMPTS.keys()), label_visibility="collapsed") [cite: 10]
    ...
    if img_file is not None:
        if st.button("✨ ブースト開始", use_container_width=True):
            ...
            with st.status("解析中...🚀"):
                ...
                # [cite_start]プロンプト仕様（完全復元 [cite: 12, 13, 14, 15]）
                ...
                【共通厳守ルール】
                1. 画像内の教科が「{final_subject_name}」に関連しない場合は is_match: false とせよ。
    ...
    if st.session_state.final_json:
        ...
        with st.container(border=True):
            ...
            with col_d:
                if is_eng and st.button("⏩ 英文再生", use_container_width=True):
        ...
        for i, q in enumerate(quizzes):
            ...
            if ans:
                ...
                if ans == correct_val:
                    [cite_start]st.success(f"⭕ 正解！"); score += 1 [cite: 27]
                [cite_start]else: st.error(f"❌ 正解は「{correct_val}」です。") [cite: 28]
