...
if not st.session_state.agreed:
    ...
    st.markdown("1.私的使用の範囲内 2.公衆送信の禁止 3.再配布の禁止")
...
1. **情報源の限定**: 提供された画像に記載されていない情報は、原則として解説に含めないでください。
2. **解説本文の純粋化**: 解説本文の中に、練習問題などは絶対に一文字も書かないこと。
...
if st.session_state.explanation:
    ...
    if st.button("▶ 全体を聴く", use_container_width=True): inject_speech_script(st.session_state.explanation, speed)
...
if st.session_state.final_json:
    ...
    for i, q in enumerate(st.session_state.final_json.get("quizzes", [])):
        ...
        ans = st.radio(f"選択 問{i+1}", q['options'], key=f"ans_{i}", label_visibility="collapsed")
