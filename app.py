...
st.markdown(f"""
    ...
    .stTitle {{ font-size: 1.8rem !important; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }}
    </style>
""", unsafe_allow_html=True)

# 履歴の永続化（LocalStorage代わりのSession保持：再入力でも消えないよう構成）
...
if not st.session_state.agreed:
    ...
    with st.container(border=True):
        st.markdown("""
        ### 【本ソフトウェア利用に関する同意事項】
        **第1条（著作権の遵守）** 利用者は、解析結果等を権利者の許可なく第三者に公開してはならないものとします。
        **第2条（AI生成物の正確性と免責）** AIの回答の正確性を保証しません。
...
with tab1:
    ...
    if img_file and st.button("✨ ブースト開始", use_container_width=True):
        ...
        with st.status("解析中...🚀"):
            ...
            image.thumbnail((1024, 1024)) # 1024px以下に縮小して高速化
            ...
            7. 年齢別ルビ: 常用漢字には振らず、難読語にのみ「漢字(かんじ)」でルビを振れ。
    ...
    if st.session_state.final_json:
        ...
        for i, q in enumerate(quizzes):
            q_id = f"q_fixed_{i}_{target_sub}"
