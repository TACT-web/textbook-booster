...
# ★追加機能：CSSによるスタイル制御（タイトル1行化とフォントサイズ連動）
st.markdown(f"""
    <style>
...
# [cite_start]--- 教科別個別プロンプト（元ファイル [cite: 1] より完全復元） ---
SUBJECT_PROMPTS = {
...
# [cite_start]--- 音声合成エンジン（元ファイル [cite: 2-7] より完全復元） ---
...
# ==========================================
# [cite_start]1. 冒頭：厳格な免責事項 ＆ 同意（元ファイル [cite: 8] を完全維持）
# ==========================================
if not st.session_state.agreed:
    ...
    if agree_check:
        ...
        if st.button("🚀 ブーストを開始する", use_container_width=True):
            if api_key_input:
                [cite_start]st.session_state.user_api_key = api_key_input [cite: 10]
...
with tab1:
    ...
    if img_file and st.button("✨ ブースト開始", use_container_width=True):
        ...
        # [cite_start]安定性を考慮し 1.5-flash を推奨（元ファイル [cite: 11] の preview 相当）
        model = genai.GenerativeModel('gemini-1.5-flash')
        
        with st.status("解析中...🚀"):
            image = Image.open(img_file)
            image.thumbnail((1200, 1200)) # 高速化のためのリサイズ
            
            # [cite_start]--- プロンプト（元ファイル [cite: 12-15] より完全復元） ---
            ...
            if not res_json.get("is_match"):
                [cite_start]st.error(f"🚫 教科不一致: {res_json['detected_subject']}"); st.stop() [cite: 16, 17]
    ...
    if st.session_state.final_json:
        ...
        with st.container(border=True):
            [cite_start]speech_speed = st.slider("🐌 音声速度調整", 0.5, 2.0, 1.0, 0.1) [cite: 19]
            ...
            for i, block in enumerate(res.get("explanation_blocks", [])):
                [cite_start]with st.container(border=True): [cite: 22]
                    ...
                    if st.session_state.show_voice_btns:
                        ...
                        with v_col2:
                            if st.button(f"🔄", key=f"ref_{i}"):
                                [cite_start]inject_speech_script(stop=True); st.rerun() [cite: 24, 25]
        ...
        for i, q in enumerate(quizzes):
            ...
            if ans:
                ...
                if ans == correct_val:
                    st.success(f"⭕ 正解！")
                    score += 1
                [cite_start]else: st.error(f"❌ 残念。正解は「{correct_val}」です。") [cite: 28]
        ...
        if answered_count == len(quizzes) and len(quizzes) > 0:
            if st.button("🏁 最終結果を記録する", use_container_width=True, type="primary"):
                ...
                # ★追加機能：JSTで履歴保存（再ログイン等でもセッション中維持）
                ...
                st.session_state.history[target_sub].append({
                    [cite_start]"date": jst_now.strftime("%m/%d %H:%M"), [cite: 30]
                    "page": user_page,
