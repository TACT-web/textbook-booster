...
if not st.session_state.agreed:
    ...
    with st.container(border=True):
        ...
        **第2条（AI生成物の正確性と免責）** 生成物の正確性を保証しません。損害について開発者は一切の責任を負いません。
        **第3条（利用目的）** 私的な学習補助を目的とします。
        """)
        agree_check = st.checkbox("同意します")
    ...
    if agree_check:
        ...
        with c1:
            ...
            grades_list = [f"{i}年生" for i in range(1, 7)]
            st.session_state.grade = st.selectbox("学年", grades_list)
        with c2:
...
with tab1:
    ...
    if subject_choice == "その他":
        custom_subject = st.text_input("教科名を入力")
        if custom_subject: final_subject_name = custom_subject

    # 標準カメラ呼び出し
    img_file = st.file_uploader("📸 撮影（全画面）", type=['png', 'jpg', 'jpeg'], capture="camera")

    if img_file is not None:
        if st.button("✨ ブースト開始", use_container_width=True):
            ...
            with st.status("解析中...🚀"):
                ...
                【共通厳守ルール】
                1. 画像が「{final_subject_name}」に関連しない場合は is_match: false とせよ。
                2. 根拠を [P.〇 / 〇行目] で付加せよ。
                3. audio_scriptはひらがな等で読みやすくせよ。
                4. 解説は{st.session_state.age_val}歳向けにせよ。
                5. 問題数は必ず「{st.session_state.quiz_count}問」生成せよ。
                ###JSON形式で出力せよ###
                {{
                    "is_match": true, "detected_subject": "{final_subject_name}", "page": "数字",
                    "explanation_blocks": [{{"text": "本文\\n解説", "audio_target": "本文"}}],
                    "audio_script": "解説台本",
                ...
                if not res_json.get("is_match"):
                    st.error(f"🚫 不一致: {res_json['detected_subject']}"); st.stop()
    ...
    if st.session_state.final_json:
        ...
        with st.container(border=True):
            ...
            with col_c:
                btn_label = "🎙️ 個別:ON" if st.session_state.show_voice_btns else "🎙️ 個別:OFF"
                if st.button(btn_label, use_container_width=True): st.session_state.show_voice_btns = not st.session_state.show_voice_btns
            with col_d:
            ...
            for i, block in enumerate(res.get("explanation_blocks", [])):
                with st.container(border=True):
                    ...
                    if st.session_state.show_voice_btns:
                        ...
                        with v1:
                            if st.button(f"▶", key=f"p_{i}"): inject_speech_script(block["audio_target"], speech_speed, is_english=is_eng)
                        with v2:
                            if st.button(f"🔄", key=f"r_{i}"): inject_speech_script(stop=True); st.rerun()
        ...
        for i, q in enumerate(quizzes):
            ans = st.radio(f"問{i+1}: {q.get('question')} ({q.get('location')})", q.get('options'), key=f"q_{i}_{target_sub}", index=None)
            if ans:
                answered_count += 1
                if ans == q.get('options')[q.get('answer')]:
                    st.success("⭕ 正解！"); score += 1
                else: st.error(f"❌ 正解は {q.get('options')[q.get('answer')]}")
